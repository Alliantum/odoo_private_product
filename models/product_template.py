...
class ProductTemplate(models.Model):
    ...
    restricted_partner_ids = fields.Many2many('res.partner',
                                              compute='_get_restricted_partner_ids',
                                              inverse='_set_restricted_partner_ids',
                                              string="Customers",
                                              help="If some contacts are attached to the current product,"
                                                   " then this could be just be sold to those customers")
    brand_partner_status = fields.Selection([('limited', 'Limited'), ('restricted', 'Restricted')], string="Brand Partner Status")

    def _get_restricted_partner_ids(self):
    ...
    def _set_restricted_partner_ids(self):
